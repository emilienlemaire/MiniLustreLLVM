...
let llvm_module = ref (Llvm.create_module llvm_context "test")
...
let update_mem mem_ptr update_vals node_name =
    ...
    List.iter (fun (attr_n , (mlval: Ast_lib.Imp_ast.atom)) ->
        ...
        with Not_found ->
            ...
            exit 1;
        in
        let gep = Llvm.build_struct_gep mem_val idx "" llvm_builder in
        let llval =
          match mlval with
          | Const c ->
              llvalue_of_const c
...
let compile_node node =
  ...
  let mem_ptr = ref (Llvm.const_null i32_ty) in
  let llvalues =if !has_mem then
    let mem_ptr_param = (Llvm.params func_val).(0) in
    let mem_ptr_alloc = Llvm.build_alloca (Llvm.pointer_type mem_struct) "mem" llvm_builder in
    mem_ptr := mem_ptr_alloc;
    let _ = Llvm.build_store mem_ptr_param mem_ptr_alloc llvm_builder in
    mem_ptr_alloc::llvalues
  else
      llvalues
  in
...
let mk_main main steps =
  ...
  let incr_block = Llvm.append_block llvm_context "incr" func_val in
  let ret_block = Llvm.append_block llvm_context "ret" func_val in

  (* entry *)
  Llvm.position_at_end entry_block llvm_builder;
  let ret = Llvm.build_alloca i32_ty "" llvm_builder in
  let idx = Llvm.build_alloca i32_ty "" llvm_builder in
  let _ = Llvm.build_store zero ret llvm_builder in
  ...
  let mem_init = if not (Ss.mem main_name (!empty_mem_nodes)) then
      ...
      let mem_ptr_alloca = Llvm.build_alloca mem_struct "" llvm_builder in
      ...
      | Some v ->
              v
      in
      let _ = Llvm.build_call init_f [| mem_ptr_alloca |] "" llvm_builder in
      Some(mem_ptr_alloca)
  else
      None
  in
  ...
  let _ = Llvm.build_br incr_block llvm_builder in

  (* incr *)
  Llvm.position_at_end incr_block llvm_builder;
  let idx_val = Llvm.build_load idx "" llvm_builder in
  ...
  let ret_val = Llvm.build_load ret "" llvm_builder in
...
let compile file_name nodes main steps =
  llvm_module := Llvm.create_module llvm_context file_name;
